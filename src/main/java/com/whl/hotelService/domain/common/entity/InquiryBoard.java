...
public class InquiryBoard extends BaseEntity {
    ...
    @JoinColumn(name = "user_id", foreignKey = @ForeignKey(name="fk_inquiry_board_user_id", foreignKeyDefinition = "FOREIGN KEY(user_id) REFERENCES user(userid) ON DELETE CASCADE ON UPDATE CASCADE"), nullable = false)
